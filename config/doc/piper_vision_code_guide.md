...
例如 30 Hz 下，0 秒采集后，0.002 秒调用会返回原帧；0.034 秒调用会生成新帧，时间戳为 0.034 秒。该函数既不睡眠，也不推进物理仿真。调用间隔过大时只采集当前状态，不补造历史帧。
...
`validate_vision_config()` 限制 RGB 为 1280×720 或 1920×1080，深度为 848×480 或 1280×720，配对帧率为 6、15 或 30 Hz。深度宽 848 时 `min_depth_m` 至少为 0.168 m；宽 1280 时至少为 0.28 m，且最小深度小于最大深度。三个误差参数必须有限且非负。
