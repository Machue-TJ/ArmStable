...
已增加 MuJoCo RGB-D 相机、绿色方块/蓝色球识别及米制三维定位。
配置、坐标系和使用方法见 [D435i 视觉说明](doc/d435i_vision.md)。
...
`base_link` 已增加六自由度自由关节。支持 Python 设置平移/旋转、
按仿真时间调用运动函数，以及加载 JSON/CSV/NPZ 轨迹。
接口、坐标约定和示例见 [浮动基座说明](doc/floating_base.md)。
